...
app.http("getSuggestion", {
  ...
  handler: async (request, context) => {
    const response = await openai.createCompletion({
      ...
      prompt:
        "Create a random prompt for DALL-E to generate a random image that will be shown to the user. Include a number of details such as the genre, the type of painting, with additional options such as: watercolor, 4k, black and white, photo-realistic, modern, abstract, oil painting, synthwave, renaissance, 3d, high definition, futuristic and more options of your choice. Do not wrap the answer in quotes.",
      max_tokens: 100,
      temperature: 0.8,
    });
